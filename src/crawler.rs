...
impl Food {
    fn new(name: &str, courses: &str) -> Self {
        Food {
            name: name.to_string(),
...
// 获取饭菜单
fn crawler_menu_md() -> Result<String, reqwest::Error> {
    reqwest::blocking::get(FOOD_MENU_URL)?.text()
}

fn source_md_to_category(md: &str) -> HashMap<String, Vec<Food>> {
    let parts: Vec<&str> = md.split("###").skip(1).collect();

    let mut categories: HashMap<String, Vec<Food>> = HashMap::new();
    for part in parts {
        let mut lines = part.lines();
        let category = if let Some(line) = lines.next() {
            line.trim().to_string()
        } else {
            continue;
        };
        // 过滤掉不要的标题
        if !FOOD_MENU_CATEGORY.contains(&category.as_str()) {
            continue;
        }
        ...
        for line in lines {
            ...
            if let Some(cpas) = re.captures(line) {
                let food = Food::new(&cpas["name"], &cpas["url"]);
                foods.push(food);
            }
        }
        categories.insert(category, foods);
    }
    categories
}
...
mod tests {
    ...
    #[test]
    fn test_crawler_menu_md() {
        match crawler_menu_md() {
            Ok(menu_md) => println!("菜单内容:\n{}", menu_md),
            Err(e) => eprintln!("获取菜单失败: {}", e),
        }
    }

    #[test]
    fn test_source_md_to_category() {
        let md = r#"
running 1 test
菜单内容:
# 程序员做饭指南
...
- [whatToEat 今天吃什么？的决策工具，帮助你快速选择合适的菜谱。](https://github.com/ryanuo/whatToEat)
        ...
        let categories = source_md_to_category(md);
        println!("解析后的分类: {:#?}", categories);
    }

    #[test]
    fn test_get_food_menu() {
        if let Ok(category) = get_food_menu() {
            println!(
                "菜谱分类: {:#?} | 菜谱类型数量: {} | 菜谱数量: {}",
                category,
                category.len(),
                category.values().flatten().count()
            );
        } else {
            println!("未能获取菜谱分类");
            assert!(false, "未能获取菜谱分类");
        }
    }
}
